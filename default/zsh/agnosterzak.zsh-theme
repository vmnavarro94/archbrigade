...
prompt_context() {
  if [[ -n "$SSH_CLIENT" ]]; then
    ...
    prompt_segment '#e0af68' '#1a1b26' "%B@$USER%b"
  fi
}
...
prompt_dir() {
  prompt_segment '#7dcfff' '#1a1b26' "%B%~%b"
}
...
prompt_time() {
  prompt_segment '#1a1b26' '#c0caf5' "%B%D{%a %e %b - %H:%M}%b"
}
...
prompt_status() {
  ...
  [[ -n "$symbols" ]] && prompt_segment '#1a1b26' default "$symbols"
}

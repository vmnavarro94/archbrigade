...
# Users can still opt-in to Cloudflare DNS using: archbrigade-setup-dns cloudflare
...
if [ -f /etc/systemd/resolved.conf ]; then
  ...
  if [ -f /etc/systemd/network/99-archbrigade-dns.network ]; then
    sudo rm -f /etc/systemd/network/99-archbrigade-dns.network
  ...
  echo "To use Cloudflare DNS, run: archbrigade-setup-dns Cloudflare"
fi